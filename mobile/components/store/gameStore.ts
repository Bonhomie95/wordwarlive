...
export const useGameStore = create<GameState>((set, get) => ({
    ...
    connectAndQueue: (token, mode = 'classic') => {
        ...
        // connectSocket fully disposes any prior socket and creates a fresh
        // one — so a socket left over from a finished match can't linger
        // and swallow the new queue_join.
        ...
        const wireUp = () => {
            ...
            // Mystery mode emits its own status event with the same shape
            // (state + waitedMs). Map it onto the same queueStatus field so
            // the matchmaking screen's timer/headline work identically.
            sock.off('mystery_queue_status').on(
                'mystery_queue_status',
                (s: { state: string; waitedMs: number }) => {
                    set({
                        queueStatus: {
                            state:
